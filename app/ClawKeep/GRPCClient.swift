...
final class GRPCClient {
    ...
    func fetchStatus() async throws -> KeepStatusModel {
        let response: Keep_V1_GetStatusResponse = try await withClient { client in
            try await client.getStatus(.init())
        }
        return mapStatus(response.status)
    }

    func fetchConfig() async throws -> Keep_V1_AppConfig {
        let response: Keep_V1_GetConfigResponse = try await withClient { client in
            try await client.getConfig(.init())
        }
        return response.config
    }

    func updateConfig(_ config: Keep_V1_AppConfig) async throws -> Keep_V1_AppConfig {
        var request = Keep_V1_UpdateConfigRequest()
        request.config = config
        let finalRequest = request
        let response: Keep_V1_UpdateConfigResponse = try await withClient { client in
    ...
    func triggerRepair() async throws {
        ...
        } as Keep_V1_TriggerRepairResponse
    }

    func restart() async throws {
        _ = try await withClient { client in
            try await client.restart(.init())
        } as Keep_V1_RestartResponse
    }

    func resetMonitoring() async throws {
        _ = try await withClient { client in
            try await client.resetMonitoring(.init())
        } as Keep_V1_ResetMonitoringResponse
    }

    func testNotify(channel: String) async throws {
        var request = Keep_V1_TestNotifyRequest()
        request.channel = channel
        let finalRequest = request
        _ = try await withClient { client in
            try await client.testNotify(finalRequest)
        } as Keep_V1_TestNotifyResponse
    }

    func startStatusStream(onEvent: @escaping @Sendable (KeepStatusModel) async -> Void, onError: @escaping @Sendable (Error) async -> Void) {
        Task.detached {
            do {
    ...
    func startLogStream(onEvent: @escaping @Sendable (String) async -> Void, onError: @escaping @Sendable (Error) async -> Void) {
        Task.detached {
            do {
                var request = Keep_V1_SubscribeLogsRequest()
    ...
    private func withClient<Result: Sendable>(_ body: @escaping @Sendable (Keep_V1_KeepService.Client<HTTP2ClientTransport.Posix>) async throws -> Result) async throws -> Result {
        try await withGRPCClient(
            ...
            let client = Keep_V1_KeepService.Client(wrapping: grpcClient)
            return try await body(client)
        }
    }

    nonisolated private func mapStatus(_ status: Keep_V1_KeepStatus) -> KeepStatusModel {
        var model = KeepStatusModel()
