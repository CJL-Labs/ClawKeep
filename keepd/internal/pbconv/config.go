...
func ConfigToProto(cfg *config.Config) *keepv1.AppConfig {
	agents := make([]*keepv1.AgentEntry, 0, len(cfg.Agent.Agents))
	for _, agent := range cfg.Agent.Agents {
		agents = append(agents, &keepv1.AgentEntry{
			Name:       agent.Name,
	...
	return &keepv1.AppConfig{
		Monitor: &keepv1.MonitorConfig{
		...
		Agent: &keepv1.AgentConfig{
			DefaultAgent: cfg.Agent.DefaultAgent,
			Agents:       agents,
		},
		Repair: &keepv1.RepairConfig{
...
func ProtoToConfig(pb *keepv1.AppConfig) *config.Config {
	cfg := &config.Config{}
	if pb == nil {
		return cfg
	}
...
func StatusToProto(status orchestrator.Status) *keepv1.KeepStatus {
	return &keepv1.KeepStatus{
...
func LogEntryToProto(entry logcollector.Entry) *keepv1.LogEntry {
	return &keepv1.LogEntry{
...
func mapState(state orchestrator.State) keepv1.State {
	switch state {
	case orchestrator.StateWatching:
		return keepv1.State_STATE_WATCHING
	case orchestrator.StateCrashDetected:
		return keepv1.State_STATE_CRASH_DETECTED
	case orchestrator.StateCollecting:
		return keepv1.State_STATE_COLLECTING
	case orchestrator.StateRepairing:
		return keepv1.State_STATE_REPAIRING
	case orchestrator.StateRestarting:
		return keepv1.State_STATE_RESTARTING
	case orchestrator.StateExhausted:
		return keepv1.State_STATE_EXHAUSTED
	default:
		return keepv1.State_STATE_UNSPECIFIED
	}
}
